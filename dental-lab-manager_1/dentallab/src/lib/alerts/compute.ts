  ...
  for (const m of materials) {
    ...
    if (qty <= 0) {
      alerts.push({
        ...
        severity: 'critical',
        title:    'Stoc Epuizat',
        message:  `${m.name} nu mai are unități în stoc.${threshold > 0 ? ` Pragul de reaprovizionare este ${threshold} ${m.unit}.` : ''}`,
        material: pick(m),
        stock_percent: 0,
      })
      ...
      alerts.push({
        ...
        severity: 'warning',
        title:    'Stoc Redus',
        message:  `${m.name} are ${qty} ${m.unit} rămas${qty !== 1 ? 'e' : ''} (${pct}% din pragul minim de ${threshold} ${m.unit}).`,
        material: pick(m),
        stock_percent: pct,
      })
    }
    ...
    if (m.expiry_date) {
      ...
      if (days < 0) {
        alerts.push({
          ...
          severity: 'critical',
          title:    'Expirat',
          message:  `${m.name} a expirat acum ${Math.abs(days)} zi${Math.abs(days) !== 1 ? 'le' : ''} (${formatted}). Scoateți din inventar.`,
          material: pick(m),
        ...
        alerts.push({
          ...
          kind:     'expiring_soon',
          severity: 'critical',
          title:    'Expiră Foarte Curând',
          message:  `${m.name} expiră în ${days} zi${days !== 1 ? 'le' : ''} (${formatted}). Utilizați sau înlocuiți imediat.`,
          material: pick(m),
          days_until_expiry: days,
        })
        ...
        alerts.push({
          ...
          severity: 'warning',
          title:    'Expiră Curând',
          message:  `${m.name} expiră în ${days} zile (${formatted}).`,
          material: pick(m),
          days_until_expiry: days,
        })
