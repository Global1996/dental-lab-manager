...
export function AddMaterialUsageDialog({ caseId, materials, onSuccess }: Props) {
  ...
  async function onSubmit(values: AddMaterialUsageValues) {
    if (wouldExceed) {
      toast.error(`Doar ${available} ${selected?.unit} disponibil${(available ?? 0) !== 1 ? 'e' : ''} în stoc.`)
      return
    }
    const result = await addMaterialToCase(caseId, values)
    if (!result.success) { toast.error(result.error); return }
    toast.success('Material adăugat la caz', { description: 'Stocul a fost dedus automat din inventar.' })
    reset(defaults)
    setSearchTerm('')
    setOpen(false)
    onSuccess?.()
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <div className="relative z-10 w-full max-w-lg rounded-2xl border bg-background shadow-2xl
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b shrink-0">
              <div>
                <h2 id="amu-title" className="text-base font-semibold">Adaugă Material la Caz</h2>
                <p className="text-xs text-muted-foreground mt-0.5">
                  Costul este capturat acum · Stocul se deduce la salvare
                </p>
              </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} noValidate className="flex flex-col overflow-hidden">
              <div className="px-6 py-5 space-y-4 overflow-y-auto flex-1">
                  ...
                  <div className="border rounded-lg overflow-hidden max-h-[200px] overflow-y-auto">
                    {filteredMaterials.length === 0 ? (
                      <div className="flex flex-col items-center gap-1 py-6 text-muted-foreground">
                        ...
                        <p className="text-xs">Niciun material găsit</p>
                      </div>
                        ...
                        >
                          ...
                          <div className="ml-3 text-right shrink-0">
                            ...
                            <p className={cn(
                              ...
                              {isOut ? 'Stoc epuizat' : `${qty} ${m.unit} disponibil${qty !== 1 ? 'e' : ''}`}
                            </p>
                          </div>
                        </button>
                      )
                ...
                {selected && (
                  <div className={cn(
                    ...
                    <div className="grid grid-cols-3 gap-3 text-center">
                      <div>
                        <p className="text-muted-foreground mb-0.5">Cost Unitar</p>
                      ...
                      <div>
                        <p className="text-muted-foreground mb-0.5">În Stoc</p>
                      ...
                      <div className={cn(
                        ...
                        <p className="text-muted-foreground mb-0.5">Total Linie</p>
                        <p className={cn('font-bold text-sm',
                    ...
                    {wouldExceed && (
                      <div className="flex items-start gap-1.5 text-destructive">
                        ...
                        <span>
                          Doar <strong>{available} {selected.unit}</strong> disponibil{available !== 1 ? 'e' : ''}.
                          Reduceți cantitatea pentru a continua.
                        </span>
                      </div>
                    )}
                    ...
                    {!wouldExceed && (
                      <div className="flex items-start gap-1.5 text-muted-foreground">
                        ...
                        <span>Costul unitar este capturat la salvare. Modificările ulterioare de preț nu afectează acest caz.</span>
                      </div>
                    )}
                  </div>
                )}
                ...
                {/* ── Quantity ─────────────────────────────────────── */}
                <FieldWrapper
                  label={selected ? `Cantitate (${selected.unit})` : 'Cantitate'}
                  htmlFor="amu_qty"
                  required
                ...
                {/* ── Notes ────────────────────────────────────────── */}
                <FieldWrapper label="Observații (opțional)" htmlFor="amu_notes" error={errors.notes?.message}>
                  <Textarea
                    {...register('notes')}
                    id="amu_notes"
                    placeholder="Nuanță, utilizare parțială, notițe speciale…"
                    rows={2}
                  />
                </FieldWrapper>
              ...
              <div className="flex items-center justify-end gap-3 px-6 py-4 border-t bg-muted/30 shrink-0">
                ...
                <button
                  ...
                  {isSubmitting ? 'Se adaugă…' :
                   selectedId && qty > 0 ? `Adaugă · ${fmtCurrency(lineCost)}` : 'Adaugă la Caz'}
                </button>
              </div>
            </form>
          </div>
