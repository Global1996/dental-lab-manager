  ...
  return (
    <div className="rounded-lg border bg-card shadow-lg px-3 py-2.5 text-xs space-y-1">
      ...
      <p className="text-muted-foreground">
        Used: <span className="text-foreground font-medium">{d.quantity_used} {d.unit}</span>
      </p>
      <p className="text-muted-foreground">
        Value: <span className="text-foreground font-medium">{formatCurrency(d.total_value)}</span>
      </p>
    </div>
  )
}
  ...
  if (!data.length) {
    return (
      <div className="flex flex-col items-center justify-center h-48 text-muted-foreground gap-2">
        ...
        <p className="text-sm">No usage data this month</p>
      </div>
    )
  }
