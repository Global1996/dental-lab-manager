...
function RemoveButton({
  ...
  function handleRemove() {
    startTransition(async () => {
      ...
      toast.success(`"${materialName}" removed · stock NOT returned (record a Return movement if needed)`)
      setConfirming(false)
    })
  }
  ...
  if (!confirming) {
    return (
      <button onClick={() => setConfirming(true)}
        ...
        title="Remove material from case">
...
export function MaterialUsageTable({ caseId, usages, editable }: Props) {
  if (usages.length === 0) {
    return (
      <div className="rounded-xl border border-dashed bg-card p-10 text-center">
        ...
        <p className="text-sm font-medium text-muted-foreground">No materials assigned yet</p>
        <p className="text-xs text-muted-foreground mt-1 max-w-xs mx-auto">
          {editable
            ? 'Use "Add Material" to assign inventory materials. Stock is deducted and costs roll up automatically.'
            : 'No materials were recorded for this case.'}
        </p>
      </div>
    )
  }
  ...
  const cols = ['Material', 'SKU', 'Qty', 'Unit Cost (snapshot)', 'Line Total', 'Notes']
              ...
              return (
                <tr key={u.id}
                  ...
                  <td className="px-4 py-3 font-medium max-w-[200px]">
                    <span className="block truncate">{u.materials?.name ?? 'Unknown'}</span>
                  </td>
